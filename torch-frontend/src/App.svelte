<script lang="ts">
    ...
    function updateChartWithJsonData(jsonData: { [key: string]: number[] }) {
        ...
        const newEpochs: string[] = Array.from({ length: size }, (_, i) => (epoch + i + 1).toString());
        ...
        Object.entries(jsonData).forEach(([key, values]) => {
            ...
            const existingDataset = datasets1.find((dataset) => dataset.label.toLowerCase() === transformedLabel);
...
<main>
    ...
    <div class="horizontal-split flex-space-filling">
        <section class="chart-section">
            ...
            <ChartComponent chartData={{ labels: labels1, datasets: datasets1 }} {chartOptions} chartType="line" />
        </section>
        <div class="list-vertical flex-space-filling">
            <section class="action-section list-vertical">
                    ...
                    <div>
                        ...
                        {:else if selectedElement.type === "int"}
                            ...
                            <div>
                                ...
                                <input id="slider-int" type="range" min="0" max="100" step="1" bind:value={dynamicValues} />
                                <input type="number" min="0" max="100" step="1" bind:value={dynamicValues} />
                            </div>
                        ...
                        {:else if selectedElement.type === "list_int"}
                            ...
                            {#each dynamicValues as value, index}
                                <div>
                                    ...
                                    />
                                    <input type="number" min="0" max="100" step="1" bind:value={dynamicValues[index]} />
                                </div>
                            {/each}
