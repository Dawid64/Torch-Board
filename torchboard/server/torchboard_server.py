...
class TorchBoardServer:
    ...
    def __init__(
        ...
        self.app.add_url_rule(
            "/get_changes", "get_changes", self.__get_changes_session, methods=["GET"]
        )
        self.app.add_url_rule(
            "/get_history", "get_history", self.__get_history, methods=["GET"]
        )
        self.app.add_url_rule(
            "/get_variables", "get_variables", self.__get_variables, methods=["GET"]
        )
        self.app.add_url_rule(
            "/update_variable",
            "update_variable",
            self.__update_variable,
            methods=["PUT"],
        )
    ...
    def start(self, start_browser=False) -> None:
        ...
        if start_browser:
            ...
            )  # Force open browser to dashboard
    ...
    @cross_origin()
    def __get_layers(self) -> flask.Response:
        layers = self.board.model.get_layers()
        return flask.jsonify(layers), 200

    @cross_origin()
    def __setup_output_hook(self, module_name: str, output_num: int = 0):
        self.board.model.register_forward_hooks(module_name, output_num)
    ...
    def __return_cached_output(self) -> flask.Response:
        if self.board.model.cache is None:
            ...
            output = self.board.model.cache
            self.board.model.cache = None
    ...
    def __get_variables(self) -> flask.Response:
        return flask.jsonify(
            {key: value for key, value in self.variable_state.items()}
        ), 200
    ...
    def __update_variable(self) -> flask.Response:
        ...
        if not name in self.variable_state:
            return flask.jsonify(
                {"status": "error", "message": f"Variable {name} not found"}
            ), 404
